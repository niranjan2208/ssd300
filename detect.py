...
def detect(original_image, test_object, min_score, max_overlap, top_k, suppress=None):
    ...
    for i in range(det_boxes_batch.size(0)):
        ...
        # Ground truth debug
        # box_location_1 = boxes[i].tolist()
        # draw.rectangle(xy=box_location_1, outline=label_color_map[det_labels_batch[i]])
        # draw.rectangle(xy=[l + 1. for l in box_location_1], outline=label_color_map[
        #     det_labels_batch[i]])
        ...
        # # Text - true box debug
        # text_size = font.getsize(true_labels[i].upper())
        # text_location = [box_location_1[0] + 2., box_location_1[1] - text_size[1]]
        # textbox_location = [box_location_1[0], box_location_1[1] - text_size[1], box_location_1[0] + text_size[0] + 4.,
        #                     box_location_1[1]]
        # draw.rectangle(xy=textbox_location, fill=label_color_map[true_labels[i]])
        # draw.text(xy=text_location, text=true_labels[i].upper()+":  " +str(round(true_labels[0][i].item(),2)), fill='black',
        #           font=font)
    del draw
...
if __name__ == '__main__':
    ...
    #                     default='data/JPEGImages/Objects_Camera0/Camera0_image_48.jpg',
    #                     help='path to the test data')
    # args = vars(parser.parse_args())
    # img_path = args['input']
    with open(os.path.join('../input_json_exp', 'TEST_images.json'), 'r') as j:
        test_images = json.load(j)
    with open(os.path.join('../input_json_exp', 'TEST_objects.json'), 'r') as j:
